...
##########
# Different N can't work
class MLPmixer(nn.Module):
    def __init__(self, indim, hiddim, outdim, N, dropout=0.1):
        ...
        self.norm1 = nn.LayerNorm(indim)
        self.norm2 = nn.LayerNorm(N)
    def forward(self, x, use_transpose=False):
        # x : [B, N, D]
        out = self.l1(self.norm1(x)) + x
        
        if(use_transpose):
            out = out.permute(0,2,1)
            out_2 = self.l2(self.norm2(out)) + out
...
class FCNet(nn.Module):
    def __init__(self,indim,hiddim,outdim, dropout=0.0):
...
class FCNet_Res(nn.Module):
    def __init__(self,indim,hiddim,outdim, dropout=0.1):
        super(FCNet_Res,self).__init__()
        ...
        self.act2 = nn.ReLU()
        self.norm2 = nn.LayerNorm(indim)
        
        # self.l3 = nn.Linear(indim, indim)
        # self.act2 = nn.ReLU()
        # self.norm2 = nn.LayerNorm(indim)
    ...
    def forward(self,x):
        out = self.act1(self.l1(x)) + x
        out2 = self.act2(self.l2(out)) + out
        # out3 = self.act2(self.l3(out2)) + out
        x = self.l4(out2)
        return x
...
class PatternEncoder_patternkeyv2(nn.Module):
    def __init__(self,pattern_dict):
        ...
        self.hour_list = list(map(str, list(pattern_dict.keys())))
        # self.K, self.D = pattern.shape
        self.temporature_dict = {}
        self.Qnet_dict, self.FCnet_dict, self.qry_mat_dict,self.TSFormer_dict = {}, {}, {},{}
        K, D = self.pattern_dict[1].shape
        
        for h in self.hour_list:
            self.pattern_dict[h] = self.pattern_dict[int(h)]
            self.temporature_dict[h] = np.sqrt(D)
            # self.Qnet = nn.Linear(self.D, self.D)
            self.Qnet_dict[h] = nn.Linear(12, D)
            self.qry_mat_dict[h] = nn.Parameter(torch.randn(D * int(h),K))
            self.FCnet_dict[h] = FCNet(D * int(h), D * 2, D ,dropout=0.0)
            EncoderLayer = nn.TransformerEncoderLayer(d_model = D, nhead=2, dim_feedforward = 128, batch_first=True)
            self.TSFormer_dict[h] = nn.TransformerEncoder(EncoderLayer, num_layers = 1)
        self.Qnet_dict, self.FCnet_dict, self.qry_mat_dict, self.TSFormer_dict = nn.ModuleDict(self.Qnet_dict), nn.ModuleDict(self.FCnet_dict), nn.ParameterDict(self.qry_mat_dict), nn.ModuleDict(self.TSFormer_dict)
        self.final_FC = FCNet(D * len(self.hour_list), D * 2, D)
    def forward(self, X):
        """
        :param X: [B, N, L, D]. D is the size of embedding corresbonding to pattern.shape[1]
        """        
        ...
        for h in self.hour_list:
            ...
            # qry : [B, N, L, D]
            qry = self.Qnet_dict[h](X)
            B, N ,L, D = qry.shape
            # qry : [B, N, L/h, D*h]
            qry = qry.reshape(B, N, L//int(h), D * int(h))
            
            # weight : [B, N, L/h, K]
            weight = torch.einsum('dk,bnld->bnlk',self.qry_mat_dict[h],qry)
            weight = F.softmax(weight,dim = 3)

            # w_X : [B, N, L/h, D*h]
            w_X = torch.einsum('bnlk,kd->bnld',weight,self.pattern_dict[h])
            # pattern_emb : [B, N, L/h, D]
            pattern_emb = self.FCnet_dict[h](w_X)
            # [B*N, L/h, D]
            pattern_emb = pattern_emb.reshape(B*N,L//int(h),-1)
            pattern_emb = self.TSFormer_dict[h](pattern_emb)
            # print("qry shape : {}, weight shape : {}, self.pattern_dict[h] shape : {}, pattern_emb shape : {}".format(qry.shape,weight.shape, self.pattern_dict[h].shape, pattern_emb.shape))
            # pattern_emb : [B, N, D]
            pattern_emb = pattern_emb.reshape(B,N,L//int(h),-1)[:,:,-1,:]
            if(final_pattern == None):
                final_pattern = pattern_emb
            else:
                final_pattern = torch.cat([final_pattern, pattern_emb], dim=-1)
        # output: [B, N, D]
        output = self.final_FC(final_pattern)
        # print(w_X.shape)
        return output

class PatternEncoder_patternkeyv3(nn.Module):
    def __init__(self,pattern_dict):
        super(PatternEncoder_patternkeyv3, self).__init__()
        # pattern [K, D]
        self.pattern_dict = pattern_dict
        self.hour_list = list(map(str, list(pattern_dict.keys())))
        # self.K, self.D = pattern.shape
        ...
        for h in self.hour_list:
            ...
            self.qry_mat_dict[h] = nn.Parameter(torch.randn(D * int(h),K))
            # self.FCnet_dict[h] = FCNet(D * int(h), D * 2, D ,dropout=0.0)
            self.FCnet_dict[h] = nn.Linear(D * int(h), D)

        self.Qnet_dict, self.FCnet_dict, self.qry_mat_dict = nn.ModuleDict(self.Qnet_dict), nn.ModuleDict(self.FCnet_dict), nn.ParameterDict(self.qry_mat_dict)
        # self.final_FC = FCNet(D * len(self.hour_list), D * 2, D)
        self.final_FC = nn.Linear(D * len(self.hour_list), D)
    def forward(self, X):
        """
        :param X: [B, N, L, D]. D is the size of embedding corresbonding to pattern.shape[1]
        """        
        ...
        for h in self.hour_list:
            ...
            qry = qry[:,:,-1,:]
            # weight : [B, N, K]
            weight = torch.einsum('dk,bnd->bnk',self.qry_mat_dict[h],qry)
            weight = F.softmax(weight,dim = 2)

            # w_X : [B, N, D*h]
            w_X = torch.einsum('bnk,kd->bnd',weight,self.pattern_dict[h])
            # pattern_emb : [B, N,  D]
            pattern_emb = self.FCnet_dict[h](w_X)
            # pattern_emb : [B, N, D]
...
class LT_model(nn.Module):
    def __init__(self, indim, hiddim, outdim):
        super(LT_model, self).__init__()
        self.l1 = nn.Linear(indim, hiddim)
        self.act = nn.ReLU()
        self.l2 = SGC(hiddim)
        self.l3 = nn.Linear(hiddim, outdim)
    def forward(self, x, A):
        # x : [B, N, k]
        x = self.l1(x)
        x = self.act(x)
