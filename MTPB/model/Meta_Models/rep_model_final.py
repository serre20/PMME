...
class PatchFSL(nn.Module):
    ...
    Full PatchFSL Model
    """
    def __init__(self, data_args, model_args, task_args, PatchFSL_cfg, args, model='GWN'):
        ...
        for hour in hour_list:
            pattern_dict[hour] = torch.load(PatchFSL_cfg['base_dir'] / 'pattern/{}/{}_{}_cl_{}.pt'.format(PatchFSL_cfg["data_list"],PatchFSL_cfg["sim"],PatchFSL_cfg["K"], hour))
            pattern_dict[hour] = pattern_dict[hour].detach().to(self.PatchFSL_cfg['device'])
            
        
        Pattern_Encoder = PatternEncoder_patternkeyv2(pattern_dict).to(args.device)

        

        #########
        # FC_model
        model_count={'patch':2, 'pattern':3}
        
        num_model = model_count[PatchFSL_cfg['patch_encoder']]
        
        #########
        # use add
        FCmodel = FCNet_Res((num_model) * model_args['mae']['out_channel'], num_model * 2 * model_args['mae']['out_channel'], task_args['maml']['pred_num'])
        
        Longterm_model = LT_model(task_args['maml']['his_num'],model_args['mae']['out_channel'], model_args['mae']['out_channel'])
        ...
        Reconsmodel_noparam = ReconstrucAdjNet_withA(args)

        # model_list
        self.model_list = nn.ModuleList([Pattern_Encoder, STmodel, FCmodel, Reconsmodel, Reconsmodel_noparam,Longterm_model])

        log_verbose(args,"[INFO]STmodel has {} params, FCmodel has {} params, Reconsmodel has {} params, Pattern_Encoder has {} params, Reconsmodel_noparam has {}, Longterm_model has {}".format(count_parameters(STmodel), count_parameters(FCmodel),count_parameters(Reconsmodel), count_parameters(Pattern_Encoder), count_parameters(Reconsmodel_noparam), count_parameters(Longterm_model)))

    
    def forward(self, data_i, A, stage = 'train'):
        Pattern_Encoder, STmodel, FCmodel, Reconsmodel, Reconsmodel_noparam, Longterm_model = self.model_list
        ...
        if(self.PatchFSL_cfg['patch_encoder'] == 'pattern'):
            ...
            #####
            # # original, use a FC to transform [B, N, L/P, P] to [B, N, L/P, d]
            # H : [B, N, 24, 12]
            H = raw_x_1day
            # day1pattern : [B, N, D]
            pattern = Pattern_Encoder(H)
            ######
            
            A_patch = Reconsmodel(pattern)
            A_patch_2 = Reconsmodel_noparam(pattern, A, A_patch)
            

        # ############
        
        S = Longterm_model(x[:,:,0, -288:], A_patch_2)
        
        ############ 
        # For GWN 
        A_list = [A_patch_2, A_patch_2.permute(0,2,1)]        
        # A_list = [A, A.permute(0,2,1)]        
        ...
        if(self.PatchFSL_cfg['patch_encoder'] == 'raw'):
            ...
            # input_features = [raw_emb,  S]
            input_features = [raw_emb, pattern, S]
...
class STRep(nn.Module):
    ...
    """
    def __init__(self, data_args, task_args, model_args,PatchFSL_cfg, args):
        ...
        self.model = PatchFSL(data_args, model_args, task_args, PatchFSL_cfg, args).to(self.device)
    ...
    def meta_train_revise(self, data_spt, matrix_spt, data_qry, matrix_qry):
            ...
            for k in range(self.update_step):
                ...
                for param in self.model.parameters():
                    if(param.requires_grad):
                        grad_param_list.append(param)
                
                # grad = torch.autograd.grad(loss, self.model.parameters(), allow_unused=True)
                grad = torch.autograd.grad(loss, grad_param_list, allow_unused=True)
                grad = list(grad)
                ...
                iiddxx = 0
                for idx,  model_param in enumerate( self.model.parameters()):
                    if(model_param.requires_grad):
                        gra = grad[iiddxx]
                        iiddxx+=1
                        model_param.data = model_param.data - self.update_lr * gra
        ...
        grad = torch.autograd.grad(model_loss, grad_param_list, allow_unused=True)
        ...
        # return MSELoss, mse, rmse, mae, mape  np.sqrt(np.mean(total_mse))
        # return model_loss.detach().cpu().numpy(),np.mean(total_mse), np.mean(total_rmse), np.mean(total_mae),np.mean(total_mape)
        return model_loss.detach().cpu().numpy(),np.mean(total_mse), np.sqrt(np.mean(total_mse)), np.mean(total_mae),np.mean(total_mape)
    ...
    def train_batch(self, start,end,source_dataset,loss_fn,opts):
        ...
        for idx in range(start,end):
            ...
            # out,y,Ax = self.model(data_i, A_gnd, stage='finetune')
    ...
    "%Y%m%d-%H%M%S")
        ...
        log_verbose(args,"Finetuned model saved in {}".format(model_path))
        ...
        optimizer = optim.Adam(grad_param_list, lr=self.meta_lr, weight_decay=1e-2)

        best_loss = 9999999999999.0
        best_model = None
        log_verbose(args,"[INFO] Enter finetune phase")
        ...
        for i in range(target_epochs):
            ...
            log_verbose(args,'Epochs {}/{}'.format(i,target_epochs))
            log_verbose(args,'in training   Unnormed MSE : {:.5f}, RMSE : {:.5f}, MAE : {:.5f}, MAPE: {:.5f}, normed MSE : {:.5f}.'.format(np.mean(total_mse), np.sqrt(np.mean(total_mse)), np.mean(total_mae),np.mean(total_mape),np.mean(total_loss)))
            ...
            if(mae_loss < best_loss):
                ...
                best_loss = mae_loss
                log_verbose(args,'Best model. Saved.')
            log_verbose(args,'this epoch costs {:.5}s'.format(time.time()-time_1))

        log_verbose(args,"[INFO] Enter test phase")
        ...
        for i in range(self.task_args['maml']['pred_num']):
            ...
            log_verbose(args,'Horizon {} : Unnormed MSE : {:.5f}, RMSE : {:.5f}, MAE : {:.5f}, MAPE: {:.5f}'.format(i,np.mean(total_mse), np.sqrt(np.mean(total_mse)), np.mean(total_mae),np.mean(total_mape)))
