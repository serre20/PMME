...
################################################################################
            ...
            # test, choose rest of data
                ...
                X = X[:, :, 288 * self.target_days:]  # 如果target_days=0，就相当于全部target了！！
    ...
    def __getitem__(self, index):
        ...
        # if 'target_maml', choose the first city and randomly choose a batch
            ...
            select_dataset = self.data_list[0]
            # print('target select_dataset', select_dataset)
            batch_size = self.task_args['batch_size'] 

            if batch_size==46:
                torch.manual_seed(42)
                permutation = torch.randperm(batch_size) #batchsize<46时必须要修改
                indices = permutation[0: batch_size]
                #print('indices', indices)
                x_data = self.x_list[select_dataset][indices] # 按照3倍的batch取值
                y_data = self.y_list[select_dataset][indices]
            ...
            #print('indices', indices)
                ...
                #print('indices', indices)
                x_data = self.x_list[select_dataset][indices:indices+batch_size] # 按照3倍的batch取值
                y_data = self.y_list[select_dataset][indices:indices+batch_size]
            '''
            if indices+batch_size+1==46:
                x_data = self.x_list[select_dataset][indices:indices+batch_size+1] # 按照3倍的batch取值
                y_data = self.y_list[select_dataset][indices:indices+batch_size+1]
            else:
                x_data = self.x_list[select_dataset][indices:indices+batch_size] # 按照3倍的batch取值
                y_data = self.y_list[select_dataset][indices:indices+batch_size]
            '''
    ...
    def __len__(self):
        ...
        if self.stage == 'target_maml' or self.stage == 'test':
            # print('self.x_list[self.data_list[0]].shape[0]', self.x_list[self.data_list[0]].shape[0])
            # print('self.data_list[0]', self.data_list[0])
