...
mkdir -p ./out_DA/${data_list}

finetune_epochs=0 
for model_name in 'SOFTS' 'iTransformer'; do  #  
    for projection in 0; do  #  
        for DAcoef in 0.1 1 10; do  #
            for classifier in 0; do  
                for seed in 1 3 5; do  
                    for DAepoch in 1500; do  
                        for xySquaredRatio in 100; do  #  
                            for MemSize in 10000 ; do  
                                for DA in 'CMMD' 'MMD' 'CORAL'; do
                                    OMP_NUM_THREADS=${threads} CUDA_VISIBLE_DEVICES=${gpu} nohup python -u train_DA_OT.py  \
                                        ...
                                        --DA ${DA} \
                                        --model_name ${model_name} > ./out_DA/${data_list}/${test_dataset}_${DA}_${model_name}_daE${DAepoch}_daC${DAcoef}_cls${classifier}_seed${seed}_Proj${projection}_MemSize${MemSize}_xyR${xySquaredRatio}.out 2>&1
                                done
                            done
                        done 
                    done 
