...
for model_name in 'MLP_noRevIn' 'SOFTS' 'iTransformer'; do  #  
    for projection in 0 ; do  #  no projectopn
        for DAcoef in 0; do  #
            for classifier in 0; do  # no DVGP used 
                for seed in 1 3 5; do  
                    for DAepoch in 50000; do  
                            ...
                            for MemSize in 10000 ; do  
                                for DA in 'OT' ; do
                                    OMP_NUM_THREADS=${threads} CUDA_VISIBLE_DEVICES=${gpu} nohup python -u train_DA_OT.py  \
                                        ...
                                        --DA ${DA} \
                                        --model_name ${model_name} > ./out/${data_list}/${test_dataset}_no${DA}_${model_name}_daE${DAepoch}_daC${DAcoef}_cls${classifier}_seed${seed}_Proj${projection}_MemSize${MemSize}_xyR${xySquaredRatio}.out 2>&1
                                done
                            done
                        done 
                    done 
