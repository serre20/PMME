...
class Logger():
    ...
    def __init__(self, opt):
        ...
        self.name = opt.checkpoint_name
        self.saved = False

        # create a logging file to store training losses
        self.log_name = os.path.join(opt.checkpoints_dir, opt.checkpoint_name, 'loss_log.txt')
        self.metrics_name = os.path.join(opt.checkpoints_dir, opt.checkpoint_name, 'metrics.txt')
        self.plot_dir = os.path.join(opt.checkpoints_dir, opt.checkpoint_name, 'plots')
    ...
    def print_current_losses(self, dataset_name, epoch, iters, losses, t_comp, t_data_loader, t_data_gpu):
        ...
        """
        message = '(epoch: %d, data: %s, iters: %d, comp: %.3fs, dataloader: %.3fs, 2gpu: %.3fs) ' % (epoch, dataset_name, iters, t_comp, t_data_loader, t_data_gpu)
        ...
        if self.opt.phase != 'test' and self.opt.enable_neptune:
            self.neptune_current_losses(epoch, iters, losses, t_comp, t_data_loader, t_data_gpu)
    ...
    def print_current_horizontal_metrics(self, metrics):
        for k, v in metrics.items():
            total_horizon = len(v)
            break
        for i in range(total_horizon):
            message = 'Horizon {} : '.format(i)
            for k, v in metrics.items():
                message += '%s: %.3f, ' % (k, v[i])
            print(message)  # print the message
            with open(self.metrics_name, "a") as log_file:
