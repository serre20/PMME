...
class KrigingPrompting(TaskPrompting):
    ...
    def cache_results(self):
        self.X = self.X[:, self.s_mti] * self.std + self.mean
        self.decoded_x = self.decoded_x[:, self.s_mti] * self.std + self.mean
        self._add_to_cache('X', self.X)
        self._add_to_cache('decoded_x', self.decoded_x)

    def compute_metrics(self):
        X = self.results['X']
        X = torch.from_numpy(X).flatten(2, 3)
        decoded_x = self.results['decoded_x']
        decoded_x = torch.from_numpy(decoded_x).flatten(2, 3)
        mae, mape, rmse = [], [], []
